...
class DELIVER(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, Tensor]:
        ...
        #     if modal in sample and torch.rand(1).item() < 0.2:  # 50% chance to set to zero
        #         sample[modal] = torch.zeros_like(sample[modal])
            # # 计算直方图
            # hist, bin_edges = torch.histogram(sample['event'].flatten().float(), bins=30)
            #
            # # 转换为numpy以便使用matplotlib绘图
            # hist = hist.numpy()
            # bin_edges = bin_edges.numpy()
            # # 绘制直方图
            # plt.figure(figsize=(10, 6))
            # plt.hist(sample['event'].flatten().float().numpy(), bins=30, alpha=0.7, color='blue', edgecolor='black')
            # plt.title('event_value_distribution')
            # plt.xlabel('Value')
            # plt.ylabel('Frequency')
            # plt.grid(True)
            # plt.savefig('/home/yi/Documents/DELIVER/semseg/datasets/debug_show/event_value_distribution.png')
            # raise Exception
