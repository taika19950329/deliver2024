...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=1, norm_layer=None, flatten=True):
        super().__init__()
...
class MoE(nn.Module):
    ...
    def __init__(self, input_size,num_experts, noisy_gating=False, k=2):
        ...
        self.k = k
        # instantiate experts
        self.experts = nn.ModuleList([PatchEmbed() for i in range(self.num_experts)])
        self.shared_expert = PatchEmbed()
        self.router = PatchEmbed(embed_dim=1)
    ...
    def noisy_top_k_gating(self, x, train, noise_epsilon=1e-2):
        ...
        print("888888", shared_x.shape, gate_1.shape)

        clean_logits = gate_1.squeeze(-1) @ self.w_gate
        print(clean_logits.shape, gate_1.squeeze(-1).shape, self.w_gate.shape)
        ...
        print(gates.shape, load.shape, shared_x.shape)
    ...
    def forward(self, x, z, loss_coef=1e-2):
        ...
        print(load.shape, shared_z.shape)
        # raise Exception
        ...
        print(expert_inputs_x[0].shape, expert_inputs_z[0].shape)
        ...
        print("expert_outputs_z", expert_outputs_z[0].shape)
...
if "__main__"==__name__:
    moe_instance = MoE(input_size=64, num_experts=4)
    tensor_1 = torch.randn(2,3,256,256)
