...
class DualBlock(nn.Module):
    def __init__(self, dim, volumn, head, sr_ratio=1, dpr=0., mlp_ratio=4., drop=0., act_layer=nn.GELU,
        ...
        # self.bi_fuse = Bi_direct_adapter(dim=dim//2, input_dim=dim)
        # self.share_fuse = Shared_direct_adapter(rank=dim)
        # self.fuse1 = BiParticFusion(dim, 8)
        self.fuse1 = TF_3D(embedding_dim=dim, volumn_size=volumn, nhead=4, method="TF")
    ...
    def forward(self, x: Tensor, y: Tensor, B, H, W) -> Tensor:
        ...
        # fusion
        # x_in = self.norm1(x).permute(0, 2, 1)
        # y_in = self.msp_norm1(y).reshape(B, C, -1)
        #
        # x_bi = self.bi_fuse(x_in)
        # y_bi = self.bi_fuse(y_in)
        #
        # xy_s = self.share_fuse(x_in, y_in)
        # yx_s = self.share_fuse(y_in, x_in)
        #
        # x = x + self.drop_path(y_bi.permute(0, 2, 1)) + self.drop_path(xy_s.permute(0, 2, 1))
        # y = y + self.msp_drop_path(x_bi.reshape(B, C, H, W)) + self.msp_drop_path(yx_s.reshape(B, C, H, W))
...
class CMNeXt(nn.Module):
    def __init__(self, model_name: str = 'B0', modals: list = ['rgb', 'depth', 'event', 'lidar']):
        ...
        if self.num_modals > 0:
            self.fusion1 = TF_3D(embedding_dim=embed_dims[0], volumn_size=256, nhead=4, method="TF")
            self.fusion2 = TF_3D(embedding_dim=embed_dims[1], volumn_size=128, nhead=4, method="TF")
            self.fusion3 = TF_3D(embedding_dim=embed_dims[2], volumn_size=64, nhead=4, method="TF")
            self.fusion4 = TF_3D(embedding_dim=embed_dims[3], volumn_size=32, nhead=4, method="TF")
        ...
        self.block1 = nn.ModuleList(
            [DualBlock(embed_dims[0], 256, 1, 8, dpr[cur + i], mlp_ratio=8, norm_cfg=norm_cfg) for i in range(depths[0])])
        self.norm1 = nn.LayerNorm(embed_dims[0])
        if self.num_modals > 0:
        ...
        self.block2 = nn.ModuleList(
            [DualBlock(embed_dims[1], 128, 2, 4, dpr[cur + i], mlp_ratio=8, norm_cfg=norm_cfg) for i in range(depths[1])])
        self.norm2 = nn.LayerNorm(embed_dims[1])
        if self.num_modals > 0:
        ...
        self.block3 = nn.ModuleList(
            [DualBlock(embed_dims[2], 64, 5, 2, dpr[cur + i], mlp_ratio=4, norm_cfg=norm_cfg) for i in range(depths[2])])
        self.norm3 = nn.LayerNorm(embed_dims[2])
        if self.num_modals > 0:
        ...
        self.block4 = nn.ModuleList(
            [DualBlock(embed_dims[3], 32, 8, 1, dpr[cur + i], mlp_ratio=4, norm_cfg=norm_cfg) for i in range(depths[3])])
        self.norm4 = nn.LayerNorm(embed_dims[3])
        if self.num_modals > 0:
